  ...
  var JSV = {
    ...
    validate: function () {
      ...
      if (data) {
        ...
        if (stop) {
          var r = ajv.validate(schema, data);
          result = {
            valid: r,
            errors: !r ? ajv.errors : [],
          };
        } else {
          result = ajv.validate(data, schema);
        }
    ...
    setInfo: function (node) {
      ...
      JSV.createPre(schema, ajv.getSchema(node.schema), false, node.plainName);
    ...
    compileData: function (schema, parent, name, real, depth) {
      ...
      var key,
        node,
        s = schema.$ref ? ajv.getSchema(schema.$ref) : schema,
        props = s.properties,
        items = s.items,
      ...
      if (s.allOf) {
        ...
        s = Object.assign(
          s,
          ...s.allOf.map((sc) => (sc.$ref ? ajv.getSchema(sc.$ref) : sc))
        );
        delete s.allOf;
        props = s.properties;
        items = s.items;
      ...
      node = {
        ...
        name:
          (schema.$ref && real ? name : false) || s.title || name || "schema",
        isReal: real,
        plainName: name,
        type: s.type,
        displayType:
     ...
     */
    diagonal1: d3.svg.diagonal().projection(function (d) {
      return [d.y, d.x];
    }),
    /**
